...
async function deleteContactMessage(id) {
  return await db("contact_messages").where({ id }).del();
}
// end here
...
// delivery profile calculate 
async function calculateProfileCompletion(accountId) {
  const account = await db("accounts").where({ id: accountId }).first();
  const profile = await db("profiles").where({ account_id: accountId }).first();
  const birthPlace = profile ? await db("birth_places").where({ profile_id: profile.id }).first() : null;
  ...
  const filledCount = fieldsToCheck.filter(f => f !== null && f !== undefined && f !== "").length;
...
// new calculator for calculating for agriculture
async function getDistinctRegionsForCalculator() {
  const rows = await db("crop_cost_benchmarks").distinct("region").orderBy("region", "asc");
  return rows.map(r => r.region);
}

async function getDistrictsByRegion(region) {
  const rows = await db("crop_cost_benchmarks").where({ region }).distinct("district").orderBy("district", "asc");
  return rows.map(r => r.district);
}

async function getCropsByRegionDistrict(region, district) {
  const rows = await db("crop_cost_benchmarks").where({ region, district }).distinct("crop_name").orderBy("crop_name", "asc");
  return rows.map(r => r.crop_name);
}

async function getCropBenchmark(region, district, cropName, farmingMethod) {
  return await db("crop_cost_benchmarks")
    .where({ region, district, crop_name: cropName, farming_method: farmingMethod })
    .first();
}

async function saveCostCalculation(accountId, data) {
  return await db("cost_calculations").insert({ account_id: accountId, ...data });
}
// end here
...
function detectClosingIntent(message) {
  const closingPhrases = [
    'ok thanks', 'okay thanks', 'thank you', 'thanks', 'that\'s all', 'thats all',
    'no more questions', 'nothing else', 'that is all', 'ok', 'okay', 'alright',
    'got it', 'perfect', 'great thanks', 'all good', 'i\'m done', 'im done', 'bye', 'goodbye',
  ];
  const lower = message.trim().toLowerCase();
  return closingPhrases.some(phrase => lower === phrase || lower.startsWith(phrase));
}
// end here.
...
/**************************
 * 
 * 1: CHANGES ADDED 01/09/2026 Delivery face id login or biometric
 */
async function saveWebauthnCredential(accountId, credentialId, publicKey, counter, deviceName) {
  const inserted = await db("webauthn_credentials").insert({
    account_id: accountId, credential_id: credentialId, public_key: publicKey, counter, device_name: deviceName,
  }).returning("*");
  return inserted[0];
}

async function getWebauthnCredentialsByAccount(accountId) {
  return await db("webauthn_credentials").where({ account_id: accountId });
}

async function getWebauthnCredentialById(credentialId) {
  return await db("webauthn_credentials").where({ credential_id: credentialId }).first();
}

async function updateWebauthnCounter(credentialId, counter) {
  return await db("webauthn_credentials").where({ credential_id: credentialId }).update({ counter });
}

async function deleteWebauthnCredential(id) {
  return await db("webauthn_credentials").where({ id }).del();
}
// END HERE
